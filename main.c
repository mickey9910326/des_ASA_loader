...
uint8_t parse_hex_data_iter(FILE *fp, void *data_p, uint16_t *data_num,
                            uint8_t *chksum);
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        ch = getopt_long(argc, argv, "p:h:", long_options, &option_index);
        ...
        case '?':
            ...
            printf(
                "Usage: cmd_ASA_loader [--port <com>] [--hex <file_name>]\n");
            restore_cur_color();
            ...
            printf("  -?                   Same as --help\n");
            printf("  --verbose            Show external messege for debug "
                   "usage\n");
            return 0;
            break;
    ...
    // core
    unsigned char start[12] = {0xfc, 0xfc, 0xfc, 0xfa, 0x01, 0x00,
                               0x04, 0x74, 0x65, 0x73, 0x74, 0xc0};
    unsigned char end[8] = {0xfc, 0xfc, 0xfc, 0xfc, 0x01, 0x00, 0x00, 0x00};
    unsigned char tri_data[265] = {0xfc, 0xfc, 0xfc, 0xfc, 0x01};
    unsigned char chk[12] = {0xfc, 0xfc, 0xfc, 0xfc, 0x01, 0x00,
                             0x04, 'O',  'K',  '!',  '!',  '?'};
    ...
    if (!nouploading)
        if (RS232_OpenComport(CPORT_NR, BDRATE, mode)) {
            ...
            printf(
                "Please check M128 is connected to USB and im program mode.");
            restore_cur_color();
            exit(EXIT_FAILURE);
        }
...
#endif
        RS_block_send(CPORT_NR, start, 12);
        sleep_ms(100);
#ifdef __WIN32
        RS_WaitCommEvent(CPORT_NR, &dwCommEvent, NULL);
#endif
        RS_block_polling(CPORT_NR, get_chk, 12);
    ...
    do {
        ...
        if (!nouploading) {
            ...
            RS_block_send(CPORT_NR, tri_data, data_count + 8);
            sleep_ms(100);
        }
        if (do_verbose) {
    ...
    print_progressbar((float)times / (float)totaltimes);
    if (!nouploading) {
        RS_block_send(CPORT_NR, end, 8);
#ifdef __WIN32
        tcdrain(int __fd) RS_WaitCommEvent(CPORT_NR, &dwCommEvent, NULL);
#endif
        RS_block_polling(CPORT_NR, get_chk, 12);
        if (!strncpy(get_chk, chk, 12)) {
            error = 1;
