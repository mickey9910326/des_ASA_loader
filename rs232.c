...
*/
  ...
  Cport[comport_number] = CreateFile(comports[comport_number],
  ...
  port_settings.XonChar = 17;
  port_settings.XoffChar = 19;
...
int RS_GetCommModemStatus(int comport_number,DWORD* dwModemStatus) {
    if (!GetCommModemStatus(Cport[comport_number], dwModemStatus) )
        return 1;
    else
        return 0;
}
int RS_GetCommState(int comport_number,DCB* dcb) {
    if (!GetCommState(Cport[comport_number], dcb) )
        return 1;
    else
        return 0;
}
